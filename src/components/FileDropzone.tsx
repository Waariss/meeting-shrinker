...
export function FileDropzone({ onFilesSelected, accept, title, helper, buttonLabel }: FileDropzoneProps) {
  ...
  return (
    <div
      ...
      className={`rounded-lg border-2 border-dashed p-6 transition sm:p-8 ${
        isDragging
          ? 'border-sea bg-sea/5'
          : 'border-ink/20 bg-white dark:border-white/15 dark:bg-[#111f27]'
      }`}
    >
      <div className="mx-auto flex max-w-xl flex-col items-center text-center">
        ...
        <h2 className="mt-4 text-xl font-semibold text-ink dark:text-white">{title}</h2>
        <p className="mt-2 text-sm leading-6 text-ink/65 dark:text-white/65">{helper}</p>
        <button
          type="button"
